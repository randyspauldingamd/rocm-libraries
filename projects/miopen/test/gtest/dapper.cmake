...
macro(dapper_define_categories)
...
# TODO Fix Dapper default minimal run if no work to do. It's in calc_union_filter but in mici this is ran by dapper_diff which does not run until after check
    ...
    if(DEFINED MIOPEN_DEBUG_CATEGORY)
        ...
        if(MIOPEN_DEBUG_CATEGORY_UPPER STREQUAL "QUICK")
            set(MIOPEN_CATEGORY_FILTER "${MIOPEN_DEBUG_DAPPER_FILTER}:${MIOPEN_CATEGORY_QUICK}")
        elseif(MIOPEN_DEBUG_CATEGORY_UPPER STREQUAL "STANDARD")
            set(MIOPEN_CATEGORY_FILTER "${MIOPEN_DEBUG_DAPPER_FILTER}:${MIOPEN_CATEGORY_STANDARD}")
        else()
        ...
        set(MIOPEN_GTEST_FILTER "${MIOPEN_CATEGORY_FILTER}")
    endif()

    message(STATUS "Using MIOPEN_CATEGORY_FILTER: ${MIOPEN_CATEGORY_FILTER}")
endmacro()

macro(dapper_add_sharded_test)
    add_test(NAME miopen_gtest_sharded_dapper
        COMMAND ${Python_EXECUTABLE} ${MIOPEN_DAPPER_DIFF} "${TESTS_JSON}" "${MIOPEN_DEBUG_CATEGORY}" "${MIOPEN_CATEGORY_FILTER}"
        WORKING_DIRECTORY ${MIOPEN_DAPPER_OUT_DIR}
    )
