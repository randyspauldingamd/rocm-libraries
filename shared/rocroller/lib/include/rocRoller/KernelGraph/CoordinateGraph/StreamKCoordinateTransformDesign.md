...
This does not need a new kind of `CoordinateTransform` edge.  It can be done with existing `Tile` and `Flatten` edges, by overriding the stride values on one of the `Tile` edges:
...
graph TD
  ...
  FT(["Linear (K iters for each D macrotile) (Size=512) (x)"])

  style MTAN0 stroke:#3f3
  style MTBN1 stroke:#3f3
  style FT stroke:#3f3

  L0(["Linear(size=120) (total D macrotiles)"])
  L1(["Linear(size=61440) (total K iters for all D macrotiles) (y)"])

  Flatten0["Flatten"]
  Flatten1["Flatten"]

  SK["Tile"]
  L3(["Linear(size=1920) (total K iters for this workgroup) (z)"])
  ...
  Init: 0
  Increment: 1
  Condition: x < size(x) && y < size(y) && z < size(z)"])

  ForTile(["For Tile
  Init: 0
  Increment: Counter += ForK counter
  Condition: z < size(z)"])

```
### Notes:
 - The For K loop will end when the K dimension hits its maximum.
    - If the final tile is a partial tile, then we will need to end when our workgroup's dimension ends (**z**).
    - If the number of workgroups does not divide the total K loop iterations, then the last workgroup's final tile will need to end when we hit the end of the problem (**y**) (although this might be handled by (**x**)).
 - The For Tile loop will contain the For K loop, and its increment will be the number of For K loop iterations.
 - The control graph would implement code roughly equivalent to:
   ```C++
   size_t forKIdx = 0;
   for(size_t forTileIdx = 0; getDimensionValue(z, {forKIdx, forTileIdx}) < getDimensionSize(z); forTileIdx += forKIdx)
   {
       forKIdx = 0;
       // initialize accumulation registers to 0
       // prefetch for-K loop
       bool isStartOfPartialTile = getDimensionValue(x, {forKIdx, forTileIdx, blockIdx.x}) > 0;
       for(;  getDimensionValue(x, {forTileIdx, forKIdx, blockIdx.x}) < getDimensionSize(x)
           && getDimensionValue(y, {forTileIdx, forKIdx, blockIdx.x}) < getDimensionSize(y)
           && getDimensionValue(z, {forTileIdx, forKIdx}) < getDimensionSize(z);
           ++forKIdx)
       {
           // body of for-K loop
       }
       
       bool isOwnerOfPartialTile = getDimensionValue(x, {forTileIdx, forKIdx, blockIdx.x}) < getDimensionSize(x);
       
       if(isStartOfPartialTile)
       {
   ...
   ```
 - The Stream-K tiles will be done in a way that is fastest within the ForTile loop of a particular workgroup.  This is difficult to change at the lower level of this graph.
    - Reordering the tiles after the flattening the MacroTileNumber dimensions (i.e. on the `Linear(size=120)` dimension), or reversing the order of the `MacroTileNumber`s would be different ways of changing the memory access pattern on this level if needed.
