...
namespace rocRollerTest
{
    ...
    TEST_F(GPU_GemmGuidePostTest, ManualKernelSmall_Minimal)
    {
        ...
        RandomGenerator random(923168u);

        unsigned long long sizeC = 4096, sizeA = 4096, sizeB = 4096;
        std::vector<float> hostD(sizeC);
        std::vector<float> hostC = random.vector<float>(sizeC, -1.f, 1.f);
        std::vector<float> hostA = random.vector<float>(sizeA, -1.f, 1.f);
        std::vector<float> hostB = random.vector<float>(sizeB, -1.f, 1.f);
    ...
    TEST_F(GPU_GemmGuidePostTest, ManualKernel_Minimal)
    {
        ...
        RandomGenerator random(219678u);
